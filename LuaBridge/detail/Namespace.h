    ...
    */
    ...
    public:
        ...
        */
        ...
        Class<T>& addFunction(char const* name, int (*mfp)(lua_State*))
        {
            assertStackState(); // Stack: const table (co), class table (cl), static table (st)

            // 存储普通函数指针到 userdata
            typedef int (*MFP)(lua_State*);
            new (lua_newuserdata(L, sizeof(MFP))) MFP(mfp); // 使用 placement new

            // 使用自定义的转发函数，而不是 CFunc::CallFunction
            lua_pushcclosure(L, &Class<T>::call_plain_function, 1);
            rawsetfield(L, -3, name); // 假设 -3 指向 static table

            return *this;
        }

        // 静态转发函数（可放在类内 private 或类外）
        static int call_plain_function(lua_State* L) {
            int (*func)(lua_State*) = *static_cast<int (**)(lua_State*)>(
